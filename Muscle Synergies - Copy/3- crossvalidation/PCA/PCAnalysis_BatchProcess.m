...
% Outcome will be a .mat file (PCA_Subj_01.mat)
    ...
    for subjectcounter = 1:size(SubjectIDs,2)
        ...
        for fold = 1:NumberFolds
            ...
            for numSynergy = 1:DOF 
                ...
                if RightVAF(numSynergy,fold)>90 & DeltaRightVAF(numSynergy,fold)<5 & RightDOF_VAF(numSynergy,4:DOF,fold)>50 & GoodTrainR==0
                    ndimR(fold,1) = numSynergy;
                    GoodTrainR = 1;
                end
                if LeftVAF(numSynergy,fold)>90 & DeltaLeftVAF(numSynergy,fold)<5 & LeftDOF_VAF(numSynergy,4:DOF,fold)>50 & GoodTrainL==0
            ...
            RightEvalApprox = (RightVal) * [PCsRightTr(:,1:ndimR(fold,1)) zeros(size(PCsRightTr,1),DOF-ndimR(fold,1))] * PCsRightTr';
            LeftEvalApprox = (LeftVal) * [PCsLeftTr(:,1:ndimL(fold,1)) zeros(size(PCsLeftTr,1),DOF-ndimL(fold,1))] * PCsLeftTr';
            ...
            AvgReconsErrRightVal(fold,1) = (1/size(RightVal,1))*sum((1/DOF*sum(((RightVal )-RightEvalApprox).^2,2)).^0.5); %8 DOF
            AvgReconsErrLeftVal(fold,1) = (1/size(LeftVal,1))*sum((1/DOF*sum(((LeftVal )-LeftEvalApprox).^2,2)).^0.5); %8 DOF
            AvgReconsErrRightTr(fold,1) = (1/size(RightTrain,1))*sum((1/DOF*sum((RightTrain-RightTrApprox).^2,2)).^0.5); %8 DOF
            AvgReconsErrLeftTr(fold,1) = (1/size(LeftTrain,1))*sum((1/DOF*sum((LeftTrain -LeftTrApprox).^2,2)).^0.5); %8 DOF

            %correlation coefficient (slope)

            for temp = 1:DOF %8 DOFs
        ...
        % %        AllRSQLeftTr(fold,temp) = stats(1,1);
            ...
            while  GoodTrainR == 0 | GoodTrainL==0;
                ...
                if ValRightVAF(numSynergy,fold)>90 & ValDeltaRightVAF(numSynergy,fold)<5 & ValRightDOF_VAF(numSynergy,4:DOF,fold)>50 & GoodTrainR==0
                    ndimRVal(fold,1) = numSynergy;
                    GoodTrainR = 1;
                end
                if ValLeftVAF(numSynergy,fold)>90 & ValDeltaLeftVAF(numSynergy,fold)<5 & ValLeftDOF_VAF(numSynergy,4:DOF,fold)>50 & GoodTrainL==0
...
%                     [coeff, bint, r, rint, stats] = regress(PCsLeftTrNorm(:,i), PCsRightTrNorm(:,j));
        ...
        %% save the analysis outcome
            ...
            save(strcat('PCA_', 'Weight_Subj_', SubjID, '.mat' ), 'RightVAF', 'LeftVAF', ...
